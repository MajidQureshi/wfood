...
.table-title .add-new-stocknorms {
    float: right;
    height: 30px;
    font-weight: bold;
    font-size: 12px;
...
.table-title .add-new-stocknorms i {
    margin-right: 4px;
}
table.table {
    table-layout: fixed;
...
$(document).ready(function(){
    ...
    $(".add-new-stocknorms").click(function(){
        // $(this).attr("disabled", "disabled");
        var index = $("table tbody tr:last-child").index();
        var row = '<tr>' +
            '<td><select name="sale_offices[]" class="form-control">'+
            ...
            '</select>'+
            '</td>' +
            '<td><select name="warehouses[]" class="form-control">'+
            '<option value="">Select</option>'+
        ...
        $("#stocknorms").append(row);     
        // $("#stocknorms tbody tr").eq(index + 1).find(".add, .edit").toggle();
        $('[data-toggle="tooltip"]').tooltip();
        $('.edit').hide();
    });
    ...
    $(document).on("click", ".add", function(){
        ...
        if(!empty){
            ...
            $(".add-new-stocknorms").removeAttr("disabled");
        }       
    });
    ...
    $(document).on("click", ".edit", function(){        
        ...
        $(this).parents("tr").find(".add, .edit").toggle();
        $(".add-new-stocknorms").attr("disabled", "disabled");
    });
...
<div class="container-lg">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Stock Norms</h2></div>
                    <div class="col-sm-4">
                        <button type="button" class="btn btn-info add-new-stocknorms"><i class="fa fa-plus"></i> Add New</button>
                    </div>
                </div>
            </div>
    <div class="table-responsive" style="overflow-y: auto;height: 300px; overflow-y: visible;">
            ...
            <table class="table" id="stocknorms">
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr>
                        <td>
                            <select name="sale_offices[]" class="form-control">
                                <option value="">Select</option>
                              @foreach($sale_offices as $sale_office)
                                <option value="{{ $sale_office['id'] }}">{{ $sale_office['title'] }}</option>
                              @endforeach
