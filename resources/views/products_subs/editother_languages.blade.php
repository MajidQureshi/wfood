...
.table-title .add-new-ol_language {
    float: right;
    height: 30px;
    font-weight: bold;
    font-size: 12px;
...
.table-title .add-new-ol_language i {
    margin-right: 4px;
}
table.table {
    table-layout: fixed;
...
$(document).ready(function(){
    ...
    $(".add-new-ol_language").click(function(){
        // $(this).attr("disabled", "disabled");
        var index = $("table tbody tr:last-child").index();
        var row = '<tr>' +
            '<td><select name="ol_language[]" class="form-control">'+
            ...
            '</td>'+
            '<td>'+
            '<input type="text" class="form-control" name="ol_short_name[]">'+
            '</td>'+
            '<td>'+
            '<input type="text" class="form-control" name="ol_details[]">'+
            '</td>'+
            '<td>'+
            '<input type="text" class="form-control" name="ol_is_active[]">'+
            '</td>'+
            '<td>' + actions + '</td>' +
        '</tr>';
        ...
        $("#ol_language").append(row);     
        $("#ol_language tbody tr").eq(index + 1).find(".add, .edit").toggle();
        $('[data-toggle="tooltip"]').tooltip();
    });
    ...
    $(document).on("click", ".add", function(){
        ...
        if(!empty){
            ...
            $(".add-new-ol_language").removeAttr("disabled");
        }       
    });
    ...
    $(document).on("click", ".edit", function(){        
        ...
        $(this).parents("tr").find(".add, .edit").toggle();
        $(".add-new-ol_language").attr("disabled", "disabled");
    });
...
<div class="container-lg">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Other Languages</h2></div>
                    <div class="col-sm-4">
                        <button type="button" class="btn btn-info add-new-ol_language"><i class="fa fa-plus"></i> Add New</button>
                    </div>
                </div>
            </div>
    <div class="table-responsive" style="overflow-y: auto;height: 300px; overflow-y: visible;">
            ...
            <table class="table" id="ol_language">
                <thead>
                    <tr>
