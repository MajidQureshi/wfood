...
.table-title .add-new-child_barcode {
    float: right;
    height: 30px;
    font-weight: bold;
    font-size: 12px;
...
.table-title .add-new-child_barcode i {
    margin-right: 4px;
}
table.table {
    table-layout: fixed;
...
$(document).ready(function(){
    ...
    $(".add-new-child_barcode").click(function(){
        // $(this).attr("disabled", "disabled");
        var index = $("table tbody tr:last-child").index();
        var row = '<tr>' +
            '<td><select name="cb_color[]" class="form-control">'+
            ...
            '</select>'+
            '</td>' +
            '<td><select name="cb_size[]" class="form-control">'+
            '<option value="">Select</option>'+
        ...
        $("#child_barcode").append(row);     
        $("#child_barcode tbody tr").eq(index + 1).find(".add, .edit").toggle();
        $('[data-toggle="tooltip"]').tooltip();
    });
    ...
    $(document).on("click", ".add", function(){
        ...
        if(!empty){
            ...
            $(".add-new-child_barcode").removeAttr("disabled");
        }       
    });
    ...
    $(document).on("click", ".edit", function(){        
        ...
        $(this).parents("tr").find(".add, .edit").toggle();
        $(".add-new-child_barcode").attr("disabled", "disabled");
    });
...
<div class="container-lg">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Child Barcodes</h2></div>
                    <div class="col-sm-4">
                        <button type="button" class="btn btn-info add-new-child_barcode"><i class="fa fa-plus"></i> Add New</button>
                    </div>
                </div>
            </div>
    <div class="table-responsive" style="overflow-y: auto;height: 300px; overflow-y: visible;">
            ...
            <table class="table" id="child_barcode">
                <thead>
                    <tr>
                        <th>Color</th>
                        <th>Size</th>                        
